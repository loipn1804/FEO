...
public class SmoothProgressDrawable extends Drawable implements Animatable {
	...
	private SmoothProgressDrawable(Interpolator interpolator, int sectionsCount, int separatorLength, int[] colors, float strokeWidth, float speed, float progressiveStartSpeed,
			float progressiveStopSpeed, boolean reversed, boolean mirrorMode, Callbacks callbacks, boolean progressiveStartActivated, Drawable backgroundDrawable, boolean useGradients) {
		mRunning = false;
	...
	public void setBackgroundDrawable(Drawable backgroundDrawable) {
		if (mBackgroundDrawable == backgroundDrawable)
			return;
		mBackgroundDrawable = backgroundDrawable;
		invalidateSelf();
	}
	...
	private void drawStrokes(Canvas canvas) {
		...
		for (int i = 0; i <= mCurrentSections; ++i) {
			...
			if (i == mCurrentSections) {
				lastX = prevValue + sectionWidth; // because we want to keep the
													// separator effect
			}
	...
	private void drawBackground(Canvas canvas, float fromX, float toX) {
		...
		canvas.clipRect(fromX, (int) ((canvas.getHeight() - mStrokeWidth) / 2), toX, (int) ((canvas.getHeight() + mStrokeWidth) / 2));
	...
	public static class Builder {
		...
		public SmoothProgressDrawable build() {
			...
			SmoothProgressDrawable ret = new SmoothProgressDrawable(mInterpolator, mSectionsCount, mStrokeSeparatorLength, mColors, mStrokeWidth, mSpeed, mProgressiveStartSpeed,
					mProgressiveStopSpeed, mReversed, mMirrorMode, mOnProgressiveStopEndedListener, mProgressiveStartActivated, mBackgroundDrawableWhenHidden, mGradients);
			return ret;
		}
